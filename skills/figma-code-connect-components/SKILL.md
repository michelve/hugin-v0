...
- Check DSAI component paths: `src/client/components/ui/` (DSAI components installed via `dsai add` — these ARE the implementation files)
- Also check `src/client/components/` for app-specific wrapper components
- Look for variant props like `variant`, `size`, `color` that match Figma variants
- DSAI components use `forwardRef` + `displayName` and have props in separate `.types.ts` files
