...
## Example 1: React + Bootstrap/DSAI Project
...
- Use Bootstrap 5 utilities and DSAI CSS custom properties for styling

## Figma Implementation Flow

1. Run get_design_context for the node
2. Run get_screenshot for visual reference
3. Map Figma colors to DSAI tokens (`var(--dsai-color-*)`)
4. Reuse DSAI components from `src/components/ui/` — install missing ones with `dsai add`
5. Validate against screenshot before completing

## Styling Rules

- IMPORTANT: Use Bootstrap 5 utility classes, not inline styles or Tailwind
- Colors use DSAI CSS custom properties: `var(--dsai-color-primary)`, etc.
- Spacing uses Bootstrap utilities: `p-3`, `mb-4`, `gap-2`
