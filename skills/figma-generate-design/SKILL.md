...
When building screens from this project's codebase:

- **Components:** DSAI components are in `src/client/components/ui/` — installed via `dsai add <name>`. Each uses `memo(forwardRef(function Name(props, ref)))` + `displayName`.
- **CSS Framework:** Bootstrap 5 (NOT Tailwind). Maps to DSAI variable bindings: `--dsai-color-*`, `--dsai-font-*`, `--dsai-shadow-*`.
- **Token Prefix:** `--dsai-*` CSS custom properties. Map these to Figma variable names when binding.
- **Token Pipeline:** DTCG JSON in `src/collections/*.json` → `dsai tokens build` → generated CSS in `src/generated/css/`.
- **Dark Mode:** Two modes — Light and Dark — mapped via `data-dsai-theme` attribute.
- **Import Path:** `@/components/ui/<name>` in code, look for matching Figma component names.

When reading source code to understand a screen, check `src/client/components/` for page-level components and `src/client/components/ui/` for DSAI primitives.
