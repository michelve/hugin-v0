...
## 12. DSAI Project Context

When building a design system from this project's codebase:

- **Token Format:** DTCG (Design Token Community Group) JSON in `src/collections/*.json` — use these as the source of truth for Phase 0 discovery and Phase 1 foundations.
- **Token Prefix:** `--dsai-*` CSS custom properties. When setting code syntax on variables, use `var(--dsai-color-primary)`, `var(--dsai-spacing-md)`, etc.
- **Token Pipeline:** `dsai tokens build` transforms DTCG JSON → CSS/SCSS/JS/TS outputs in `src/generated/`.
- **Color System:** Semantic colors aliased to primitives — `--dsai-color-primary`, `--dsai-color-danger`, `--dsai-bg-surface`, `--dsai-color-neutral-*`.
- **Spacing:** Bootstrap 5 scale (0–5) mapped to DSAI tokens — `--dsai-spacing-xs` through `--dsai-spacing-2xl`.
- **Dark Mode:** Two modes (Light/Dark) — align Figma variable modes to `data-dsai-theme="light"` and `data-dsai-theme="dark"`.
- **Component Pattern:** `memo(forwardRef(function Name(props, ref)))` + `displayName`. Props in separate `*.types.ts` files.
- **Component Location:** `src/client/components/ui/` (DSAI primitives), `src/client/components/` (app-specific).
- **Installation:** `dsai add <component>` to install missing components.
- **CSS Framework:** Bootstrap 5 (NOT Tailwind). When mapping code styles to Figma, translate `d-flex` → Auto Layout, `gap-3` → spacing variable, `rounded` → radius variable.

Use `src/collections/*.json` files as the authoritative token source during Phase 0 discovery.
